...
class ContractDetailViewModel : BaseViewModel<List<AttachmentResponse>>() {
    ...
    private val getAttachmentsUseCase = InjectionUseCase.provideGetAttachmentByContractCodeUseCase()

    fun getAttachmentsBy(contractCode: BigInteger) {
        getAttachmentsUseCase.getAttachmentsBy(contractCode, object: UseCase.UseCaseCallback<List<AttachmentResponse>>{
